	...
	while(fgets(line,sizeof(line),fp)) {
		...
		switch (line[0]) {
			...
			case 'f':
			{
				...
				// 1st vertex
				vert.px = (scalar_t)position[i1-1].x;
				vert.py = (scalar_t)position[i1-1].y;
				vert.pz = (scalar_t)position[i1-1].z;

				vert.u = has_uvs ? (scalar_t)uv_coord[i2-1].x : 0.0f;
				vert.v = has_uvs ? (scalar_t)uv_coord[i2-1].y : 0.0f;
				
				vert.nx = has_normals ? (scalar_t)normal[i3-1].x : 0.0f;
				vert.ny = has_normals ? (scalar_t)normal[i3-1].y : 0.0f;
				vert.nz = has_normals ? (scalar_t)normal[i3-1].z : 0.0f;

				vertices.push_back(vert);
				indices.push_back(idx);

				// 2nd vertex
				vert.px = (scalar_t)position[i4-1].x;
				vert.py = (scalar_t)position[i4-1].y;
				vert.pz = (scalar_t)position[i4-1].z;

				vert.u = has_uvs ? (scalar_t)uv_coord[i5-1].x : 0.0f;
				vert.v = has_uvs ? (scalar_t)uv_coord[i5-1].y : 0.0f;

				vert.nx = has_normals ? (scalar_t)normal[i6-1].x : 0.0f;
				vert.ny = has_normals ? (scalar_t)normal[i6-1].y : 0.0f;
				vert.nz = has_normals ? (scalar_t)normal[i6-1].z : 0.0f;
	
				vertices.push_back(vert);
				indices.push_back(idx+1);

				// 3rd vertex
				vert.px = (scalar_t)position[i7-1].x;
				vert.py = (scalar_t)position[i7-1].y;
				vert.pz = (scalar_t)position[i7-1].z;

				vert.u = has_uvs ? (scalar_t)uv_coord[i8-1].x : 0.0f;
				vert.v = has_uvs ? (scalar_t)uv_coord[i8-1].y : 0.0f;

				vert.nx = has_normals ? (scalar_t)normal[i9-1].x : 0.0f;
				vert.ny = has_normals ? (scalar_t)normal[i9-1].y : 0.0f;
				vert.nz = has_normals ? (scalar_t)normal[i9-1].z : 0.0f;
